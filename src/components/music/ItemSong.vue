<template>
  <div @click="handleClickSong" class="flex flex-col gap-2 p-3 hover:rounded-xl hover:bg-neutral-800 max-h-[290px] relative">
    <div class="w-40 h-40 bg-cover">
      <img :src="songData.cover_url" alt="cover" class="rounded-lg h-full w-full" />
    </div>
    ...
    <button v-show="isVisible === songData.id" class="rounded-full w-13 h-13 p-2 bg-green-500 absolute left-30 top-30 hover:scale-105 hover:bg-green-400 cursor-pointer">
